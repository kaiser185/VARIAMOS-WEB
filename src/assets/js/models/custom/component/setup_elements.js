            ...
            if(oncreation_allowed){
                ...
                new_cells.forEach(element => {
                    ...
                    if(["package"].includes(type)){
                        element.groupElement = graph.insertVertex(element, null, '', 0, 0, 0, 0, '');
                        element.setConnectable(false);
                    }
                });
                graph.setSelectionCells(new_cells);

                //Remove groupElement if cell is a package.
                if(cell !==  undefined && cell !== null && cell.getAttribute('type') === 'package' && cell.groupElement !== undefined){
                    graph.removeCells([cell.groupElement]);
                    cell.groupElement = undefined;
                }
