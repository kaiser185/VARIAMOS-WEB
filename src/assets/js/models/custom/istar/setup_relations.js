let setup_istar_relations = function setup_istar_relations(graph, relations, 
  relation_styles) {
  graph.connectionHandler.insertEdge = function (parent, id, value, source, 
    target, style) {
    ...
    const dependum =
      (
        ["goal-dependum", "quality-dependum", "task-dependum", 
          "resource-dependum"].includes(sourceType) && 
        source.getAttribute("dependum") === "true"
      ) || (
        ["goal-dependum", "quality-dependum", "task-dependum", 
          "resource-dependum"].includes(targetType) &&
        target.getAttribute("dependum") === "true"
      );

    //rel_secobjective_secconstraint
    const satisfies = 
      ["secobjective"].includes(sourceType) && 
      ["secconstraint"].includes(targetType);
    //rel_secmechanism_secobjective || rel_secmechanism_vulnerability
    const impl = 
      ["secmechanism"].includes(sourceType) && 
      ["secobjective", "vulnerability"].includes(targetType);
    //rel_vulnerability_goal || rel_attack_vulnerability
    const affects = 
      (
        ["vulnerability"].includes(sourceType) && 
        ["goal", "quality", "task", "resource"].includes(targetType)
      ) || (
        ["attack"].includes(sourceType) && 
        ["vulnerability"].includes(targetType)
      );
    ...
    if (relations) {
      for (let i = 0; i < relations.length; i++) {
        if (relations[i]["rel_source_target"] == "and") {
          if (
            (relations[i]["source"].indexOf(source.getAttribute("type")) > -1) && 
            (relations[i]["target"].indexOf(target.getAttribute("type")) > -1)
          ) {
            for (let j = 0; j < relations[i]["attributes"].length; j++) {
              node.setAttribute(
                relations[i]["attributes"][j]["name"], 
                relations[i]["attributes"][j]["def_value"]
              );
            }
          }
        }
    ...
    if (relation_styles) {
      for (let i = 0; i < relation_styles.length; i++) {
        if (relation_styles[i]["rel_source_target"] == "and") {
          if (
            (relation_styles[i]["source"].indexOf(source.getAttribute("type")) > -1) && 
            (relation_styles[i]["target"].indexOf(target.getAttribute("type")) > -1)
          ) {
    ...
    if (dependum || satisfies || impl || affects) {
      ...
      const capitald = 
        graph.insertVertex(
          cell, null, null, 0, 0, size, size, 
          'shape='+shape+';fillColor=#FFFFFF;rotation='+angle+';selectable=0;'
        );
